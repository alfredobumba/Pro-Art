    ...
    <script type="text/javascript">
        function validaCampos() {
            if (document.fmAgencia.txtAgencia.value == "") {
                Swal.fire({
                    title: 'Erro!',
                    text: 'Por favor, preencha o nome da Agência.',
                    icon: 'error',
    ...
    <main class="container mt-5">
            ...
            <div class="col-md-9 col-sm-9">
                ...
                if (isset($_GET['eliminarAgencia'])) {
                    ...
                    if ($res = mysqli_query($con, $sql)) {
                        $res = mysqli_query($con, "SELECT @saida AS saida, @rotulo AS rotulo");
                    ...
                    <form name="fmAgencias" method="get" action="editaAgenciaAdm.php" onsubmit="return validaCampos()">
                        <label for="txtAgencia">Nome da Agência:</label><br>
                        <input type="text" id="txtAgencia" name="txtAgencia" value="<?php echo $nomeAgencia; ?>"
                               class="form-control" maxlength="50"><br>
                        <button type="submit" class="btn btn-primary w-100" name="btnSubmitAgencia">Alterar Agência
                        </button><br>
                    </form>
                    <hr/>
        ...
        if ($res = mysqli_query($con, $sql)) {
            ...
            while ($reg = mysqli_fetch_assoc($res)) {
                if (isset($reg['nome_agencia'])) {
                    $agencias[] = $reg['nome_agencia'];
                }
            }
            mysqli_free_result($res); // Libera o resultado da memória
            if (isset($agencias[0])) {
